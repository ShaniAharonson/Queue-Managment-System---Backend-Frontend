...
export function updatePatientAction(patientToUpdate: Patient): PatientAction {
    return { type: PatientActionType.updatePatient, payload: patientToUpdate };

}

export function makingAppointmentAction(newAppointment: Appointment): PatientAction {
...
export function patientAppointmentsByDoctorTypeAction(appointments: Appointment[]): PatientAction {
    return { type: PatientActionType.patientAppointmentsByDoctorType, payload: appointments };
}

export function singlePatientAction(patient: Patient): PatientAction {
    return { type: PatientActionType.singlePatient, payload: patient };
}


export function PatientReducer(currentState: PatientState = new PatientState(), action: PatientAction): PatientState {
    let newState = { ...currentState };
    switch (action.type) {
        case PatientActionType.deleteAppointment:
            newState.allPatientAppointments = [...newState.allPatientAppointments].filter((item) => item.id !== action.payload);
            break;
        case PatientActionType.updatePatient:
            newState.allPatients = [...newState.allPatients].filter((item) => item.id !== action.payload.id);
            newState.allPatients = [...newState.allPatients, action.payload];
            break;
        case PatientActionType.makingAppointment:
            newState.allPatientAppointments = [...newState.allPatientAppointments, action.payload];
            break;
        ...
        case PatientActionType.patientAppointmentsByDoctorType:
            newState.allPatientAppointments = [...newState.allPatientAppointments].filter((appointment)=>appointment.doctorType ===action.payload);
            break;
